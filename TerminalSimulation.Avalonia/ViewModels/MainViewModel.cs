...
namespace TerminalSimulation.Avalonia.ViewModels
{
    ...
    public partial class LogMessageItem : ObservableObject
    {
        ...
        private void CopyRaw()
        {
            ...
            {
                // TODO: Implement cross-platform clipboard via TopLevel.GetTopLevel
                try { System.Diagnostics.Debug.WriteLine($"[Clipboard] {RawData}"); } catch { }
            }
        }
        ...
        private void CopyFullLog()
        {
            ...
            // TODO: Implement cross-platform clipboard via TopLevel.GetTopLevel
            try { System.Diagnostics.Debug.WriteLine($"[Clipboard] {fullText}"); } catch { }
        }
    }
    ...
    public partial class MainViewModel : ObservableObject, IDisposable, TerminalSimulation.PluginBase.Avalonia.IPluginContext
    {
        ...
        private void SimulationTimerCallback(object? state)
        {
            ...
            {
                Dispatcher.UIThread.Invoke(() => SimulationTimerCallback(state));
                return;
            }
        ...
        private readonly string ConfigFile = System.IO.Path.Combine(TerminalSimulation.Avalonia.Helpers.PathHelper.ExeDir, "config.json");
        ...
        private LocationReportSnapshot CaptureLocationReportSnapshot()
        {
            if (!Dispatcher.UIThread.CheckAccess())
            {
                return Dispatcher.UIThread.Invoke(() => CaptureLocationReportSnapshotInternal());
            }
        ...
        private AppConfig CaptureAppConfig()
        {
            if (!Dispatcher.UIThread.CheckAccess())
            {
                return Dispatcher.UIThread.Invoke(() => CaptureAppConfigInternal());
            }
        ...
        private void ToggleAutoReport()
        {
            ...
            else
            {
                ...
                _ = Task.Run(async () =>
                {
                    ...
                    {
                        Dispatcher.UIThread.Invoke(() => IsAutoReporting = false);
                    }
                }, token);
            }
        }
        ...
        public void StartPathSimulation(List<GeoPoint> path)
        {
            ...
            _ = Task.Run(async () =>
            {
                ...
                {
                    Dispatcher.UIThread.Invoke(() => IsPathSimulating = false);
                }
            }, token);
        }
